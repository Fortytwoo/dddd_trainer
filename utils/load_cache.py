...
class LoadCache(Dataset):
    def __init__(
        ...
        self.tensor_cache_path = os.path.join(cache_dir, "tensors")
        self.use_tensor_cache = os.path.exists(self.tensor_cache_path)
        if self.use_tensor_cache:
            logger.info("\nUsing tensor cache from: {}".format(self.tensor_cache_path))
    ...
    def __getitem__(self, idx):
        try:
            ...
            # 尝试从 tensor 缓存读取
            if self.use_tensor_cache:
                cache_name = image_name.replace(".", "_").replace("/", "_").replace("\\", "_") + ".pt"
                cache_file = os.path.join(self.tensor_cache_path, cache_name)
                if os.path.exists(cache_file):
                    cached_data = torch.load(cache_file, weights_only=True)
                    # 将 tensor 转回 PIL Image
                    image = torchvision.transforms.ToPILImage()(cached_data['tensor'])
                    if not self.word:
