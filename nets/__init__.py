...
class Net(torch.nn.Module):
    ...
    def export_onnx(
        ...
        """
        导出 ONNX 模型，兼容 opset_version=12
        禁用 dynamo 和新特性以确保与旧版本 ONNX Runtime 的兼容性
        """
        import warnings
        ...
        try:
            # 尝试禁用 torch.compile 和 dynamo（PyTorch 2.0+）
            ...
            # 旧版 PyTorch 不支持 dynamo 参数，使用基础导出
            torch.onnx.export(
                net,
                dummy_input,
                graph_path,
            ...
            print(f"ONNX 导出遇到问题: {e}")
            print("尝试使用最小配置导出...")
            # 最后的尝试：最小配置导出
            torch.onnx.export(
                net, dummy_input, graph_path, export_params=True, opset_version=12
            )
